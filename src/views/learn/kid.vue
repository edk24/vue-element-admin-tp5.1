  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <p>
        ...
        <el-select
          v-model="listQuery.category_two"
          style="width: 140px"
          class="filter-item"
          @change="handleFilter"
        >
    ...
    >
      ...
      >
        <template slot-scope="{row,$index}">
          <!-- <el-button type="success" size="mini" @click="playVideo(row)">查看视频</el-button> -->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="playDialogFormVisible">
      <div class="whole">
        <video class="video" style="display: block;margin: 0 auto;" :src="temp.video" controls :poster="temp.images" />
        <!-- <video
          id="my-video"
          ...
          :poster="temp.images"
        >
          <source :src="temp.video" type="application/x-mpegURL" />
        </video> -->
      </div>
    </el-dialog>
  </div>
...
const grade = [{ key: '小学' }, { key: '初中' }, { key: '高中' }]
export default {
  ...
  methods: {
    ...
    getCateTwo() {
      category
        .getlist(1, 9999, '', 'learn', this.listQuery.category)
        .then(({ code, msg, data, count }) => {
    ...
    getList() {
      ...
      learn
        .list(
          ...
          this.listQuery.category_two
        )
        .then(({ code, msg, data, count }) => {
          if (code === 0) {
            data.forEach(row => {
              row.images = this.imgsrc + row.images
              row.video = this.imgsrc + row.video
              this.list.push(row)
            })
            this.total = count
          } else {
