  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <p>
        ...
        <el-select v-model="listQuery.kid_type" style="width: 180px" class="filter-item" @change="handleFilter">
          <el-option key="all" label="全部" value="all" />
          <el-option v-for="item in kid_type" :key="item.key" :label="item.name" :value="item.key" />
        </el-select>
      </p>
    </div>
    ...
    >
      ...
      <el-table-column label="分类" prop="category">
        <template slot-scope="{row}">
          <span v-if="row.kid_type === '0'">心灵成长</span>
          <span v-if="row.kid_type === '1'">思维成长</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" width="500" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="padding:0 5px;margin-right:5px;margin-left:50px;">
        ...
        <el-form-item label="视频分类" prop="category">
          <el-select v-model="temp.kid_type" style="width: 180px" class="filter-item">
            <el-option v-for="item in kid_type" :key="item.key" :label="item.name" :value="item.key" />
          </el-select>
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        this.list = []
        learn.list(this.listQuery.page, this.listQuery.limit, this.listQuery.keyword, this.listQuery.type, '', '', this.listQuery.kid_type).then(({ code, msg, data, count }) => {
          if (code === 0) {
            data.forEach(row => {
